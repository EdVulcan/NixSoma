...
static int handle_event(void *raw_context, void *raw_data, size_t data_size) {
  ...
  const struct openclaw_file_open_event *event = raw_data;
  ...
  printf("{\"timestampNs\":\"%llu\",\"pid\":%u,\"uid\":%u,\"comm\":",
    (unsigned long long)event->timestamp_ns, event->pid, event->uid);
  print_json_string(event->comm);
  printf(",\"flags\":\"%llu\",\"mode\":\"%llu\"}\n",
    (unsigned long long)event->flags, (unsigned long long)event->mode);
...
int main(int argc, char **argv) {
  ...
  for (index = 1; index < argc; index += 1) {
    if (strcmp(argv[index], "--object-path") == 0 && index + 1 < argc) object_path = argv[++index];
  ...
  if (libbpf_get_error(object)) { object = NULL; goto cleanup; }
  if (bpf_object__load(object) != 0) goto cleanup;
  program = bpf_object__find_program_by_name(object, "record_file_open");
  if (!program) goto cleanup;
  link = bpf_program__attach(program);
  if (libbpf_get_error(link)) { link = NULL; goto cleanup; }
