# NixSoma Capability Map

Updated: 2026-08-01

This is the canonical progress view. Historical phase numbers and test counts
do not determine completion. A capability advances only when its operator
workflow, owner, durable state, recovery boundary, Observer readback, and
representative evidence agree.

## Current Baselines

| Baseline | Maturity | Meaning |
| --- | --- | --- |
| Physically deployed bounded product | about 84-87% | Generation 109 carries the governed control plane, trusted work view, fixed Level 3 body owners, and bounded Level 4 eye-hand workflows. |
| Current source bounded product | about 86-89% | Adds the renewable mission supervisor, real nine-service development proof, exact Core/Observer Nix closures, and a protected-path-clean physical candidate; activation is a separate gate. |
| Final whitepaper vision | about 56-62% | Includes durable autonomous task supply and planning, evidence-based adaptation, broader body nerves, physical Phase D evolution, and richer native workflows. |

## Capability Families

| Family | Status | Current evidence | Remaining product gap |
| --- | --- | --- | --- |
| Level 1 control plane | Source frontier | Finite sessions, restart checkpoints, one-shot scheduling, bounded window leases, and renewable missions with finite epochs, explicit renewal, pause/cancel, restart re-arm, and no-progress circuit break | A mission still consumes only already-reviewed queued work; it cannot invent goals, create an open-ended backlog, retry failures, or change policy |
| Level 2 trusted work view | Deployed | User-owned helper, isolated browser/workspace, fresh bounded capture, stale-authority rejection, recovery, takeover, and Observer projection | Broader native application workflows, not another capture or lease variant |
| Level 3 controlled system body | Partially deployed | Fixed systemd owners, exact Polkit/peer boundaries, repair loop, resource evidence, process/network/file/process-lifecycle nerves | Physical Phase D mutation/rollback, wider fixed body nerves, and governed action composition |
| Level 4 graphical body | Deployed bounded subset | Dedicated compositor/browser, native frame/input, semantic and OCR actions, bounded run/assessment, reviewed acceptance, submit, and current-tab close | Multi-application native workflows and longer mission-bound eye-hand work without arbitrary desktop control |
| Memory and adaptation | Partial | Durable experience records, task-bound recall consumption, recommendation application/execution/outcome, and explicit operator feedback | Controlled experiments and counterfactual evidence sufficient to justify adaptive ranking; no causal claim or automatic authority change exists |
| Declarative evolution | VM-proven | Candidate generation, approval binding, activation, health oracle, exact rollback, and replay rejection in disposable KVM | Physical-host promotion policy and an independently authorized automatic rollback boundary |

## Newly Usable Source Workflow

```text
operator arms finite mission authority
-> supervisor checkpoints one epoch before execution
-> existing bounded window owner consumes at most one 1-20 step session
-> Observer shows progress, checkpoint, deadline, circuit, and stop reason
-> mission pauses/cancels at an epoch boundary
-> Core restart requires exact explicit re-arm
-> operator may renew only another finite epoch/deadline grant
```

The mission accepts 1-32 epochs initially, at most 32 more per renewal, and at
most 256 during one mission lifetime. Each authority grant is at most seven
days and cannot extend the live horizon beyond 30 days. It is renewable, not
uninterruptible or unbounded.

## Next Vertical Route

After deployment review for the mission supervisor, the next
missing autonomy behavior is a reviewed finite mission worklist. It should let
the operator bind a bounded set of task blueprints to one mission and let the
mission issue only the next reviewed item at an epoch boundary. This closes the
task-supply gap without giving a provider open-ended task creation or policy
authority. Adaptive ranking, physical Phase D, and wider body actions remain
separate subsequent routes.
