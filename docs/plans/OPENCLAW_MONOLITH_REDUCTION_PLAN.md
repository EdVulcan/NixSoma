...
8. Added a Phase 99-117 live-provider result-envelope milestone manifest that
...
9. Replaced the Phase 99-117 core/Observer wrapper bodies with a shared
   manifest-driven wrapper helper while preserving every public script filename
   and registry entry.
10. Extended the Phase 99-117 manifest check to derive each phase primary
...
12. Extended the manifest check to derive and verify the repeated Phase 99-117
    core/system-heal state artifact filenames in common checks.
13. Extracted the repeated Phase 99-117 common-check environment setup into a
    manifest-driven helper for ports, plan docs, state artifacts, and core /
    Observer URLs.
14. Extracted the repeated Phase 99-117 fast prerequisite reuse setup into a
    bounded helper while preserving each milestone's local service assertions
    and fallback predecessor script.
15. Replaced the hand-written Phase 99-117 core / Observer milestone registry
...
| P1 | `services/openclaw-core/src/cloud-live-provider-runtime-implementation.mjs` | Now composes extracted live-provider runtime lanes through focused factories. The Phase 18-29, Phase 32-37, Phase 40-48, Phase 52-55, Phase 56-59, Phase 60-64, Phase 65-70, Phase 71-78, Phase 79-86, Phase 87-90, and Phase 91-117 late credential/result-envelope runtime lanes are extracted into focused modules. Remaining coupling is the broad top-level dependency bag shared by all runtime lane factories. | Treat as substantially reduced. Consider a shared approved-deferred lifecycle helper only if future phase work repeats status/evidence mutation across multiple lane modules; otherwise return to Observer startup/DOM descriptors or milestone assertion helpers. |
...
| P1 | `nix/scripts` milestone script taxonomy | The runner registry is externalized, and the Phase 99-117 live-provider result-envelope core/Observer registry rows and wrappers are metadata-backed. Common-check assertions still grow by hand and include long filenames plus repeated endpoint/status checks. | Use the `milestone-script-audit` artifact as the baseline, then extract assertion helpers only for repeated checks that can preserve real service evidence and public check names. |
...
| P2 | `apps/observer-ui/src/client-script-refreshers-cloud.mjs` | Cloud/live-provider refreshers are now split into context/provider rehearsal, live runbook/runtime adapter, real launch/credential authorization, local-read, and result-envelope chunks, and startup refresh registration is descriptor-backed. The Phase 99-106 result-envelope route/task-shell/approved/final-readiness quartets now use a descriptor-backed endpoint-to-DOM helper. Remaining coupling is the repeated endpoint-to-DOM rendering pattern in later result-envelope quartets and other phase-family chunks. | Extend the proven result-envelope descriptor pattern to adjacent Phase 107-117 quartets while preserving global function names and served `/client.js` token compatibility. |
...
2. Extend the Observer result-envelope descriptor pilot across the remaining
   Phase 107-117 quartets in adjacent slices, preserving global refresh names,
