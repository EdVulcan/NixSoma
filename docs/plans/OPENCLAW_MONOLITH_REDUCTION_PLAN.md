...
8. Added a Phase 99-131 live-provider result-envelope milestone manifest that
...
9. Replaced the Phase 99-131 core/Observer wrapper bodies with a shared
   manifest-driven wrapper helper while preserving every public script filename
   and registry entry.
10. Extended the Phase 99-131 manifest check to derive each phase primary
...
12. Extended the manifest check to derive and verify the repeated Phase 99-131
    core/system-heal state artifact filenames in common checks.
13. Extracted the repeated Phase 99-131 common-check environment setup into a
    manifest-driven helper for ports, plan docs, state artifacts, and core /
    Observer URLs.
14. Extracted the repeated Phase 99-131 fast prerequisite reuse setup into a
    bounded helper while preserving each milestone's local service assertions
    and fallback predecessor script.
15. Replaced the hand-written Phase 99-131 core / Observer milestone registry
...
95. Extracted the repeated Phase 99-131 Observer HTML/client token assertions
...
104. Extended the same post-Phase-118 runtime lane through Phase 120 by adding
    the approval-gated local-read result-envelope task shell, POST route, and
    delegated executor handler in the new focused module. The slice keeps task
    creation, approval, deferred completion, Observer tokens, and milestone
    assertions in the Phase 119+ lane instead of growing the older Phase
    115-118 local-read module.
105. Extended the Phase 119+ runtime lane through Phase 122 by adding the
    approved-deferred local-read result-envelope readback route plus the final
    readiness preflight route/record path and Observer descriptor without
    introducing a new route hub branch, executor branch, or non-manifest
    milestone wrapper.
106. Started the next Phase 123+ route lane in a focused
    `cloud-live-provider-runtime-credential-local-read-result-envelope-creation-execution-attempt-local-read-result-envelope-creation.mjs`
    module instead of growing the already bounded Phase 119-122 module past its
    cohesive route/task/readback/preflight responsibility.
107. Extended the Phase 123+ focused lane through Phase 124 by adding the
    approval-gated result envelope creation task shell, delegated task handler,
    Observer surface, and manifest-backed milestone wrappers without growing
    the older Phase 119-122 runtime module.
108. Extended the Phase 123+ focused lane through Phase 125 by adding
    approved-deferred readback, Observer evidence, and manifest-backed
    validation while keeping credential reads, result envelope creation, and
    provider/network egress deferred.
109. Extended the Phase 123+ focused lane through Phase 126 by adding final
    readiness preflight record/readback, Observer evidence, and
    manifest-backed validation while preserving Phase 125 readback after the
    readiness marker and keeping credential reads, result envelope creation,
    provider/network egress, rollback, host mutation, launch, and live calls
    deferred.
110. Extended the Phase 123+ focused lane through Phase 127 by adding the
    local-only result envelope creation execution route and Observer evidence
    without adding another task/executor branch to the current slice. Phase
    128 task-shell work should consider a new sibling runtime lane if it would
    turn this bounded route/readback/preflight module into a catch-all.
111. Added the Phase 128 result envelope creation execution task-shell as a
    new sibling runtime lane rather than growing the Phase 123-127 module,
    preserving the route/readback/preflight boundary while adding approval
    gated task creation, delegated task-handler registration, Observer
    evidence, and manifest-backed milestone wrappers.
112. Extended the Phase 128 sibling execution lifecycle through Phase 129 by
    adding approved-deferred readback and GET route evidence in the same
    cohesive task-shell/readback module, while preserving no credential reads,
    no result envelope creation, and no provider/network egress.
113. Extended the Phase 128 sibling execution lifecycle through Phase 130 by
    adding final-readiness preflight GET/POST recording while keeping Phase 129
    approved-deferred readback valid after the task implementation status is
    advanced to the recorded state.
114. Started a new Phase 131 focused route runtime module for the next
    execution-attempt route instead of growing the Phase 128-130 task-shell
    lifecycle module, preserving the recorded readiness boundary and all
    credential/no-egress safeguards.
...
| --- | --- | --- | --- |
| P0 | `nix/scripts` late credential-value/result-envelope phase chain | Phase number, slug, predecessor, artifact path, registry, status marker, docs token, observer token, endpoint, and `next.recommendedSlice` are now manifest-backed across the Phase 99-131 chain. Observer HTML/client token assertions now use a manifest-derived helper. Remaining repetition lives mostly in phase-specific JSON business assertions. | Keep extending the manifest lane for adjacent result-envelope phases and extract assertion helpers only when they preserve local service evidence and reduce repeated token wiring. |
...
| P1 | `services/openclaw-core/src/cloud-live-provider-runtime-implementation.mjs` | Now composes extracted live-provider runtime lanes through focused factories. The Phase 18-29, Phase 32-37, Phase 40-48, Phase 52-55, Phase 56-59, Phase 60-64, Phase 65-70, Phase 71-78, Phase 79-86, Phase 87-90, and Phase 91-131 late credential/result-envelope runtime lanes are extracted into focused modules. Remaining coupling is the broad top-level dependency bag shared by all runtime lane factories. | Treat as substantially reduced. Consider a shared approved-deferred lifecycle helper only if future phase work repeats status/evidence mutation across multiple lane modules; otherwise return to Observer startup/DOM descriptors or milestone assertion helpers. |
...
| P1 | `nix/scripts` milestone script taxonomy | The runner registry is externalized, and the Phase 99-131 live-provider result-envelope core/Observer registry rows and wrappers are metadata-backed. Common-check assertions still grow by hand and include long filenames plus repeated endpoint/status checks. | Use the `milestone-script-audit` artifact as the baseline, then extract assertion helpers only for repeated checks that can preserve real service evidence and public check names. |
