...
| P1 | `services/openclaw-system-sense/src/server.mjs` | Bounded filesystem operations and command/process operations are extracted into focused modules, but the server still owns service discovery, health trends, route-aware recommendations, systemd inventory/dependency maps, and body evidence route reviews. | Continue extracting system-sense domain modules behind stable endpoints: health/systemd sensing, body-evidence route builders. |
