...
| P1 | `services/openclaw-core/src/plan-builder.mjs` | Plugin plans, runtime activation, cloud live-provider wiring, body evidence append/readiness read models, task creation, approval creation, event publishing, and phase evidence builders still share one builder. The capability registry/invocation slice, systemd/body task-shell builders, and Phase 7 long-term memory lane are now extracted. | Next safe boundaries are cloud-consciousness task/materialization lanes and the separate body evidence append/readiness read-model lane; keep the returned plan-builder method names stable while moving one cohesive family at a time. |
