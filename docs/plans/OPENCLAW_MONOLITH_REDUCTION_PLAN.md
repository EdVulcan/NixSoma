...
| P0 | `services/openclaw-core/src/task-executor.mjs` | Execution dispatch still imports many delegated cloud `is...Task`/`execute...Task` pairs, but task matching uses a `NON_RECOVERABLE_TASK_HANDLERS` registry instead of an ordered if/else branch chain. System/body execution now lives in `task-executor-system-body-handlers.mjs`, and native/search-web deferred runtime handlers now live in `task-executor-native-deferred-handlers.mjs`. Remaining coupling is generic work-view execution, capability invocation, read models, and broad cloud delegated handler wiring. | Continue handler group extraction only for cohesive lanes that carry their predicates, private helpers, and registry entries together; next likely slice is generic work-view/capability/read-model helpers or a cloud delegated handler registry helper, not individual functions. |
