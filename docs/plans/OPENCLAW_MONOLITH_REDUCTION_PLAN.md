...
| P2 | `apps/observer-ui/src/client-script-runtime-actions.mjs` | Runtime/system/body/systemd refreshers are extracted into `client-script-refreshers-runtime.mjs`, approval-gated workspace/native-adapter/search-web task creation now lives in `client-script-runtime-approval-tasks.mjs`, and system/body task-shell creation now lives in `client-script-runtime-system-body-tasks.mjs`; these splits preserved byte-for-byte assembled `/client.js` output. Remaining coupling is action/button registration, event stream refresh fan-out, task focus state, work-view helpers, and task completion/recovery flows. | Extract event-to-refresh and button-action descriptor tables while preserving global function names and byte-for-byte or token-equivalent served output where feasible. |
