...
| P1 | `services/openclaw-system-sense/src/server.mjs` | Bounded filesystem operations, command/process operations, and read-only systemd inventory/dependency mapping are extracted into focused modules, but the server still owns service discovery, health trends, route-aware recommendations, systemd repair planning chains, and body evidence route reviews. | Continue extracting system-sense domain modules behind stable endpoints: health trend/recommendation builders, systemd repair planning chains, body-evidence route builders. |
