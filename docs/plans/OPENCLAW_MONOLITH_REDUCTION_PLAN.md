...
45. Extracted the early `cloud-live-provider-runtime-implementation.mjs`
    ...
    shells. The main runtime implementation now composes those factories while
    preserving public method names, registry strings, endpoint behavior,
    approval gates, event publication, persistence calls, and deferred executor
    summaries.
...
| P1 | `services/openclaw-core/src/cloud-live-provider-runtime-implementation.mjs` | Still mixes real-launch review/preflight, egress execution gates, and credential-value authorization/local-read ladders with registry constants, approval gates, task phase mutation, persistence, event publication, and repeated deferred execution envelopes. The Phase 18-29, Phase 32-37, Phase 40-48, and Phase 52-55 runtime lanes are now extracted into focused factories. | Continue lane extraction only where it removes cross-phase coupling. Next cohesive target is the real-launch route/preflight surface, then the credential-value gate/read ladders. Separately consider an approved-deferred lifecycle helper that preserves status strings. |
