...
45. Extracted the early `cloud-live-provider-runtime-implementation.mjs`
    ...
    shell, executor, and approved-deferred readback. The main runtime
    ...
    publication, persistence calls, task predicates, and deferred executor
    summaries.
...
| P1 | `services/openclaw-core/src/cloud-live-provider-runtime-implementation.mjs` | Still mixes Phase 65+ credential-value authorization, credential-value read, access-authorization decision, and local-read ladders with registry constants, approval gates, task phase mutation, persistence, event publication, and repeated deferred execution envelopes. The Phase 18-29, Phase 32-37, Phase 40-48, Phase 52-55, Phase 56-59, and Phase 60-64 runtime lanes are now extracted into focused factories. | Continue lane extraction only where it removes cross-phase coupling. Next cohesive target is the Phase 65-78 credential-value authorization/read/access-decision ladder, followed by the credential-value local-read ladders. Separately consider an approved-deferred lifecycle helper that preserves status strings. |
