...
| P0 | `services/openclaw-core/src/route-handlers.mjs` | Core HTTP dispatch, proxying, phase route mapping, plugin routes, task POST routes, approval surfaces, and workspace/body routes are all in one request chain. Route tokens must be hand-synced with runtime exports, Observer refreshers, and milestone scripts. Phase 99-115 result-envelope GET routes and both live-provider POST route groups are now grouped, but non-provider route families and read-model routes still live in the main chain. | Continue route group extraction while keeping `registerRoutes(deps)` stable. Next cohesive target is the Observer/core read-model GET cluster for capabilities, command transcripts, filesystem changes, and filesystem reads because it is read-only and has shared response shapes. |
