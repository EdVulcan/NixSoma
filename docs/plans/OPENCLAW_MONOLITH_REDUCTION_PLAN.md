...
| P0 | `services/openclaw-core/src/route-handlers.mjs` | Core HTTP dispatch, proxying, phase route mapping, plugin routes, task POST routes, approval surfaces, and workspace/body routes are all in one request chain. Route tokens must be hand-synced with runtime exports, Observer refreshers, and milestone scripts. Phase 99-115 result-envelope GET routes, both live-provider POST route groups, and the Observer/core read-model GET routes are now grouped, but non-provider route families, approval actions, workspace/body routes, and policy/task mutation routes still live in the main chain. | Continue route group extraction while keeping `registerRoutes(deps)` stable. Next cohesive targets are narrow mutable families with shared dependencies, such as approval action routes or task lifecycle mutation routes, because each can be moved behind a focused handler without changing endpoint contracts. |
