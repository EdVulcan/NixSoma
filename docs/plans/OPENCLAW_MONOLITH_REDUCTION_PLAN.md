...
| P1 | `services/openclaw-system-sense/src/server.mjs` | Bounded filesystem operations, command/process operations, read-only systemd inventory/dependency mapping, health/body-governance read models, body-evidence timeline/ledger route-review builders, repair-candidate planning, next systemd repair planning, and generic repair proposal/dry-run helpers are extracted into focused modules. File, command/process, and systemd HTTP route adapters are also extracted, so the server now mostly owns service discovery, state refresh, health/body/body-evidence route dispatch, refresh events, and service lifecycle wiring. | Continue extracting route registration groups for already-extracted system/body evidence surfaces, starting with body-evidence route-review adapters and then health/body state routes. |
