...
| P1 | `services/openclaw-system-sense/src/server.mjs` | Bounded filesystem operations, command/process operations, read-only systemd inventory/dependency mapping, health/body-governance read models, body-evidence timeline/ledger route-review builders, repair-candidate planning, and the next systemd repair planning chain are extracted into focused modules. The server still owns service discovery, generic systemd repair plan/dry-run helpers, body/system HTTP route dispatch, and broad endpoint registration. | Continue extracting system-sense domain modules behind stable endpoints: generic systemd repair plan/dry-run helpers, then route registration groups for already-extracted system/body evidence surfaces. |
...
7. Continue splitting remaining `openclaw-system-sense` generic repair
   plan/dry-run helpers and route registration groups, then return to the highest-coupling
