...
45. Extracted the early `cloud-live-provider-runtime-implementation.mjs`
    ...
    build/record behavior, and both approval-gated executors. The main runtime
    implementation now
...
| P1 | `services/openclaw-core/src/cloud-live-provider-runtime-implementation.mjs` | Still mixes Phase 79+ credential-value local-read ladders with registry constants, approval gates, task phase mutation, persistence, event publication, and repeated deferred execution envelopes. The Phase 18-29, Phase 32-37, Phase 40-48, Phase 52-55, Phase 56-59, Phase 60-64, Phase 65-70, and Phase 71-78 runtime lanes are now extracted into focused factories. | Continue lane extraction only where it removes cross-phase coupling. Next cohesive target is the Phase 79-86 credential-value local-read route/task/final-readiness/execution ladder, followed by later local-read attempt and result-envelope ladders. Separately consider an approved-deferred lifecycle helper that preserves status strings. |
