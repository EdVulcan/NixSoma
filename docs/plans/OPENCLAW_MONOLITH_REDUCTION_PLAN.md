...
| P1 | `services/openclaw-core/src/plugin-review.mjs` | Workspace/source intake and Plugin SDK contract/source/native-contract lanes are now extracted into focused modules. Remaining coupling still bundles native adapter status, manifest/capability planning, search-web runtime activation, provider sandbox task creation, and workspace tool catalog/prompt/semantic/symbol/edit-target review in one composition surface. | Continue splitting review surfaces into manifest/capability, search-web runtime, and workspace semantic-index/tool-catalog modules while keeping public plugin review methods stable. |
