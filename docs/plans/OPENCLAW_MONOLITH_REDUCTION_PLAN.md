...
| P1 | `services/openclaw-core/src/plan-builder.mjs` | Plugin plans, runtime activation, systemd repair, body evidence, long-term memory, cloud live-provider wiring, task creation, approval creation, event publishing, and phase evidence builders still share one builder. The capability registry/invocation slice is now extracted. | Next safe boundaries are systemd/body evidence task-shell builders, long-term-memory builders, and cloud-consciousness task/materialization lanes; keep the returned plan-builder method names stable while moving one cohesive family at a time. |
