...
45. Extracted the early `cloud-live-provider-runtime-implementation.mjs`
    ...
    deferred execution, and execution preflight recording. The main runtime
    implementation now composes those factories while preserving public method
    names, registry strings, endpoint behavior, approval gates, event
    publication, persistence calls, and deferred executor summaries.
...
| P1 | `services/openclaw-core/src/plan-builder.mjs` | Body evidence append/readiness read models, Phase 2-6 read-only evidence builders, generic rule-plan helpers, task creation, approval creation, event publishing, and phase evidence builders still share one builder. The native plugin planning/runtime shell lane, capability registry/invocation slice, systemd/body task-shell builders, Phase 7 long-term memory lane, Phase 8 cloud handoff lane, Phase 9 provider dry-run lane, Phase 10 provider-call rehearsal lane, Phase 11 live-provider runbook lane, Phase 12 live-provider execution-plan lane, and Phase 13-17 live-provider runtime-readiness lane are now extracted. | Next safe boundary is the separate body evidence append/readiness read-model lane, followed by older Phase 3-6 read-only evidence families; keep the returned plan-builder method names stable while moving one cohesive family at a time. |
| P1 | `services/openclaw-core/src/cloud-live-provider-runtime-implementation.mjs` | Still mixes credential-value access gates, endpoint network egress gates, egress execution route/task preflight, and credential-value authorization/local-read ladders with registry constants, approval gates, task phase mutation, persistence, event publication, and repeated deferred execution envelopes. The Phase 18-29, Phase 32-37, Phase 40-48, Phase 52-55, and Phase 56-59 runtime lanes are now extracted into focused factories. | Continue lane extraction only where it removes cross-phase coupling. Next cohesive target is the credential-value and endpoint/egress gate surface, followed by the credential-value local-read ladders. Separately consider an approved-deferred lifecycle helper that preserves status strings. |
