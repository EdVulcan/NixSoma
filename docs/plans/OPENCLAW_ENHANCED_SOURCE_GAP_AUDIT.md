...
| `microcompact` | absorbed as evidence plus bounded in-memory projection | `sense.openclaw.engineering_context.microcompact_evidence` estimates reclaimable command-transcript context; `act.openclaw.engineering_context.microcompact_projection` transforms a bounded caller-owned message copy, protects recent assistant turns and verification/recovery evidence, and emits summary-only audit metadata without mutating persisted logs or task state. | Reuse the pure projection from a cohesive future context assembly owner. Do not silently mutate persisted transcripts or hide current verification/recovery evidence. | Level 1 |
...
Classification: absorbed as evidence plus bounded in-memory projection.

Recommendation:

- Reuse the pure projection from a cohesive context assembly owner. It must not
