...
| Live plugin runtime refresh | partially absorbed as a fixed native generation lifecycle | The approval-gated refresh task builds and validates a new fixed built-in registry generation, atomically swaps it into the shared native plugin plan owner, retains the previous process-lifetime generation, and makes later capability plans report the active generation id, sequence, and hash. Observer and task readback retain the approved execution. | Keep arbitrary module discovery/import, plugin code execution, loader-cache invalidation, generation persistence, and automatic rollback deferred until a governed loader and recovery owner exist. | Level 1 |
