...
Status: source and isolated local-service checks complete; physical generation
...
- A direct source probe on the current physical host reports the previous boot
  as `explicit_reboot_sequence` with reboot and shutdown markers. It does not
...
- Physical generation switch and deployed-host route proof.
...
After separately authorized physical deployment, perform one physical
read-only route check and then return to canonical route review. Do not add
another boot marker variant until a concrete operator gap requires it.
