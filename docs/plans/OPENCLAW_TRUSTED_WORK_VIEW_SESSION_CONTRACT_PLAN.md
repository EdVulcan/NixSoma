...
deployed continuation permits one fixed-step vertical scroll only on the
...
The active-surface vertical-scroll continuation is deployed in
...
candidate and normal production gates scrolled fixture surface 2 in both
directions and kept all production services healthy. Browser Runtime now has a `PartOf` relationship to
the isolated graphical-session unit, preventing a stale headed-browser
attachment after compositor restart. Active generation `b6qjcbfc...` passed the
normal lifecycle gate with durable audit, anonymous rejection, matching
post-frames, zero relevant restarts, no failed units, and no warning journals.
