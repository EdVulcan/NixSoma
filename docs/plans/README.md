...
| [`OPENCLAW_REVIEWED_WORKLIST_WORKFLOW_SELECTION_PLAN.md`](./OPENCLAW_REVIEWED_WORKLIST_WORKFLOW_SELECTION_PLAN.md) | Source-bound finite selection among four fixed reviewed workflows, immutable selection hashes, shared execution ownership, and legacy generic-worklist compatibility. |
| [`OPENCLAW_REVIEWED_WORKFLOW_ACCEPTANCE_PLAN.md`](./OPENCLAW_REVIEWED_WORKFLOW_ACCEPTANCE_PLAN.md) | Explicit hash-bound operator acceptance that reconciles a completed selected workflow with reviewed worklist advancement, with restart fail-closed behavior. |
| [`OPENCLAW_RESIDENT_AUTONOMOUS_CONTINUATION_PLAN.md`](./OPENCLAW_RESIDENT_AUTONOMOUS_CONTINUATION_PLAN.md) | Explicit resident continuation for a reviewed finite mission while workflow acceptance is pending, with finite polling and restart-required re-arm. |
...
| [`OPENCLAW_SEMANTIC_FORM_WORKFLOW_PLAN.md`](./OPENCLAW_SEMANTIC_FORM_WORKFLOW_PLAN.md) | Source-validated reviewed task-bound semantic type -> exact verified continuation -> eligible submit workflow with a two-call/two-action hard stop. |
...
before experience recording. The execution continuation in
...
The reviewed task work-view continuity slice is implemented in
[`OPENCLAW_REVIEWED_TASK_WORKSPACE_CONTINUITY_PLAN.md`](./OPENCLAW_REVIEWED_TASK_WORKSPACE_CONTINUITY_PLAN.md).
