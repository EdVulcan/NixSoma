...
| Test batch reuse P0/T1 | Every selected milestone restarted the full service stack. | Partial, result-envelope lane plus expanded core/Observer pair reuse complete | Commit `eff4beb`; `openclaw-live-provider-result-envelope-batch-reuse` milestone; `openclaw-live-provider-result-envelope-phase-115-116-batch-reuse` covers the old bespoke 115-116 batch through the manifest runner; table-driven `openclaw-core-observer-pair-batch-reuse` covers compatible Phase 59-64 non-result-envelope core/Observer pairs. Broader multi-phase lane batching remains pending. |
...
- `openclaw-core-observer-pair-batch-reuse` is table-driven and covers Phase 59-64 cloud-consciousness launch/credential/egress pairs outside the result-envelope lane.
