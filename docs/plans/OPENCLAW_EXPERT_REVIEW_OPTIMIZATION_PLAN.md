...
| Event Hub audit-log memory | Development audit queries and summaries read and split the entire JSONL file; the default `.artifacts` log has reached hundreds of MiB and can create a large transient heap spike. | Open, mainline blocker | `services/openclaw-event-hub/src/server.mjs` `readAuditEvents()` and `buildAuditSummary()` use whole-file `readFile`; replace this with bounded streaming/index/retention behavior and focused large-log tests before the next Level 3 feature slice. |
...
## Native Engineering Context Packet Pair Evidence

- `dev-openclaw-native-engineering-context-packet-pair-batch-reuse-check.sh` reuses `dev-openclaw-core-observer-pair-runner.sh` for the native context packet core and Observer common check.
- The Observer half explicitly restarts the existing session-manager session before its recovery assertion. It proves a real recovery-state readback without adding a production reset route or fault-injection endpoint.
- `OPENCLAW_MILESTONE_CHECKS=openclaw-native-engineering-context-packet-pair-batch-reuse bash nix/scripts/dev-milestone-check.sh` passed in 7 seconds with one scoped service lifecycle, one core behavior check, and one Observer HTML/client check.
- The same fresh environment previously took 5 seconds for each standalone check. The public standalone scripts remain unchanged for compatibility and release-level isolation.
- `dev-milestone-select-changed-checks.sh` maps context packet runtime/common-check changes to the pair batch, so routine `@changed` validation does not replay both standalone service lifecycles.
...
1. Remove the Event Hub whole-file audit read/split path and establish bounded retention or indexing with large-log regression coverage.
...
4. Keep moving remaining core helper-module event names behind the shared registry as those modules are touched.
