...
Level 4 已从第一个所有权边界推进到六个连续切片：登录用户的
...
画面投影与应用生命周期已通过源码、958 项工作区测试、body-config/store
...
接管、键盘/滚轮/热键、任意输入设备、任意进程/窗口 API、GNOME 输入、root 与
...
| Level 4 图形栈内生组件 | 约 45%（六个连续切片已部署并物理证明） | 已有 user-owned、资源受限的 nested compositor 和固定 Wayland socket；AI-owned Nix Firefox 已作为 headed client 接入；有界 1280x720 原生帧只保留摘要；由 lease、grant、fresh frame、audit、peer cgroup 和 post-frame 共同约束的固定左键点击已通过物理证明。认证后显式选择、浏览器内存态的 output 投影已经验收；Weston 最小 surface identity 与固定 Nix-managed Workbench 的 grant/audit start-stop 闭环也已由真实 Observer 证明。任意进程/窗口控制和桌面接管仍未完成。 |
