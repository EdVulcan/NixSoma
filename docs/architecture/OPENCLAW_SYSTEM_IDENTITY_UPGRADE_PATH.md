...
| Level 4 图形栈内生组件 | 约 10%（首个 compositor owner 已部署） | 已有 user-owned、资源受限、headless nested compositor、固定 Wayland socket，以及 session-manager/Observer 所有权与健康证据；已证明与 GNOME `wayland-0` 共存且无 parent display/DRM 访问。AI-owned browser attachment、原生像素/输入、投影与接管仍未实现。 |
