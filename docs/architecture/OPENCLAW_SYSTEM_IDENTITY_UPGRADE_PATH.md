...
使旧 frame 失效；成功证据只包含严格减少 1 的 tab count，不包含 tab id 或 URL。
...
- 该 Phase C 切片已部署到当前物理 generation `1xh4x8ls...`；Core/Observer gate
  捕获 27 个验证 `curl` 连接尝试，只包含非零 family，不包含目标、端口、payload
...
- 下一真实 Level 4 workflow 已把同一 reviewed task 的已验证 write-only
  ...
  保留 receipt，不发送 ordinal 或文本。物理 generation `5zlz2s6z...` 已用唯一精确
  Browser Runtime local fixture URL 完成 type -> submit、可见 completion、no replay、
  no plaintext、task unchanged、durable audit 与健康验收；该 lane 已冻结，不新增
  actuator、provider wrapper、通用 Enter/keyboard、任意 loopback URL 或开放循环
- 分开记录源码完成、验证通过、system generation 部署和真实物理动作
- Level 1 的真实 task composer 之后新增 bounded operator work session：操作者先
  选择 1-20 task limit，可只读预览精确 next queued task，再显式启动一次有限队列；
  ...
  仍未启用
- reviewed browser task 现在可通过显式 selected-task bridges 进入既有
...
| Level 4 图形栈内生组件 | 约 92%（source bounded scope） | 物理 baseline 已有 native/operator input、semantic type/click、reviewed run/assessment、OCR actions、current-tab close 与 receipt-bound submit；源码又新增 browser type/submit、固定 native intake、同一 exact reviewed task 下固定 browser -> native 的完整有限 mission，以及 reviewed-worklist workflow selection/reconciliation。Enter/hotkey/repeat、任意进程/窗口控制和桌面接管仍未完成。 |
...
审计和恢复证据已经形成可用子集，剩余主要是物理/live 证据、完整工作流产品化
整合与白皮书中的长期自治能力。这一比例不能替代最终愿景比例。
...
只读 `sched_process_exec`、`fentry/__sys_connect` 网络连接尝试以及
`fentry/do_sys_openat2` 文件打开尝试三个 eBPF 探针切片。网络切片只读取 family，
文件切片只读取 flags/mode 并忽略 filename 指针；三个切片均已完成物理部署。Phase D
...
session，最多 20 步，使用一个不可延长的 1 秒至 24 小时 deadline。Core restart
把 `running` 标为 blocked、把 `armed` 标为 paused；只有原 lease id 的显式
re-arm 可以恢复，且不自动重放任务、不保存输入或 provider 内容。Timer 默认关闭，
因此它证明的是受治理的有限 continuation，不是 whitepaper 所说的常驻自治。
...
其后的受控适应切片把同一 task type 的 provider context 私下分配到 baseline
与 feedback-weighted recall order。每两个 assignment 保持随机起点和一比一平衡，
provider 看不到 arm/experiment id，terminal result 必须反向匹配既有 provider task
中的 assignment hash。只有完整 8-32 trial、绝对 effect 至少 0.25 且 two-sided
Fisher exact `p <= 0.05` 才产生候选；操作者还必须提交 exact evidence hash 才能
激活或撤销 profile。它不改变 policy、approval、provider authority、actuator 或 host
authority，也不声称一般因果关系。源码 Core `963/963`、Observer `118/118`、真实
Core/Observer gate、body-config 与 `284/114` Nix closure 已通过；物理部署与 live
experiment 未发生。契约见
...
其后的 Level 4 fixed native intake workflow 增加第二个非自动启动的固定应用
`nixsoma-ai-native-intake.service`。该 Weston terminal shell 只在进程内保留最多 32
个 `[A-Za-z0-9 .,_-]` 字符，显示后不落盘、不执行命令，地址族只允许 `AF_UNIX`。
Core 在同一 AI single-flight 内要求单元先停止，经过 execution grant 与 durable
pre-action audit 启动，绑定其 matching activated surface/inventory，把该内部期望绑定
传给既有 objective-bound OCR Type owner，核验 native receipt、newer frame、post-OCR
和 completion audit 后强制 stop。start/type/stop 分开计数；任何 unknown cleanup 都
终止且不会重放输入。契约见
`../plans/OPENCLAW_NATIVE_INTAKE_WORKFLOW_PLAN.md`。
当前证据为全部 `1427/1427` workspace tests、typecheck、Core `980/980`、
Observer `120/120`、Session Manager `85/85`、真实 Core/Observer capability
...
当前源码证据为全部 `1457/1457` workspace tests、typecheck、Core `1008/1008`、
Observer `122/122`、Session Manager `85/85`、真实 Core/Observer capability gates、
...
一个既有治理 semantic click 只有在 trusted handoff、Screen Act 执行与 post-action
verification 全部成立后才生成 execution receipt，权威 completed/failed 终态再通过
outcome v1 绑定 application 与 execution receipt 并进入 experience memory。该链证明
精确动作执行与终态相关性，但仍不证明建议有效或因果关系，也不会自动调权、改变
策略或训练。
...
工作流更广的任务规划和桌面能力；这些缺口解释了最终愿景估算仍为
60-65%。
