...
| Level 4 图形栈内生组件 | 约 15%（compositor 已部署，browser attachment 待 generation 部署） | 已有 user-owned、资源受限的 nested compositor 和固定 Wayland socket；现有 AI-owned Nix Firefox 已在源码与独立 real probe 中以 headed client 接入，并复用 session-manager/Observer 证据。完整部署后的 capture/action 连续性、原生 compositor 像素/输入、投影与接管仍未完成。 |
