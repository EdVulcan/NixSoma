...
| [OPENCLAW_NATIVE_ENGINEERING_LSP_LIFECYCLE_READINESS_DRAFT_PLAN.md](./plans/OPENCLAW_NATIVE_ENGINEERING_LSP_LIFECYCLE_READINESS_DRAFT_PLAN.md) | Active draft of a governed workspace-scoped LSP lifecycle action without server startup, task creation, approval creation, state persistence, or JSON-RPC. |
| [OPENCLAW_NATIVE_ENGINEERING_LSP_SUPERVISED_LIFECYCLE_PILOT_PLAN.md](./plans/OPENCLAW_NATIVE_ENGINEERING_LSP_SUPERVISED_LIFECYCLE_PILOT_PLAN.md) | Active approval-gated LSP lifecycle task and binary gate pilot with task readback, audit events, Observer controls, and recovery recommendation while process start and JSON-RPC remain deferred. |
