...
| [OPENCLAW_NATIVE_PLUGIN_RUNTIME_REFRESH_TASK_PLAN.md](./plans/OPENCLAW_NATIVE_PLUGIN_RUNTIME_REFRESH_TASK_PLAN.md) | Active approval-gated native plugin runtime refresh task that recomputes read-model evidence after approval without module import, code execution, runtime activation, or cache mutation. |
...
| [OPENCLAW_NATIVE_ENGINEERING_PLAN_TODO_WORKBENCH_STORAGE_PLAN.md](./plans/OPENCLAW_NATIVE_ENGINEERING_PLAN_TODO_WORKBENCH_STORAGE_PLAN.md) | Active governed core-state workbench storage and local whitelisted suggested-action Observer bridge for visible plan/todo state without hidden mode, todo-file writes, task mutation, arbitrary endpoints, commands, or providers. |
...
| [OPENCLAW_TRUSTED_WORK_VIEW_SESSION_CONTRACT_PLAN.md](./plans/OPENCLAW_TRUSTED_WORK_VIEW_SESSION_CONTRACT_PLAN.md) | Active Level 1 to Level 2 bridge that carries the trusted AI-owned work-view session contract through session, browser, screen-sense, core, and Observer state. |
