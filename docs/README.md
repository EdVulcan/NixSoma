...
| [OPENCLAW_NATIVE_ENGINEERING_LSP_EVIDENCE_PLAN.md](./plans/OPENCLAW_NATIVE_ENGINEERING_LSP_EVIDENCE_PLAN.md) | Active LSP availability and lifecycle evidence lane for `cc_lsp`, including governed process probe readback while JSON-RPC remains deferred. |
| [OPENCLAW_NATIVE_ENGINEERING_LSP_LIFECYCLE_READINESS_DRAFT_PLAN.md](./plans/OPENCLAW_NATIVE_ENGINEERING_LSP_LIFECYCLE_READINESS_DRAFT_PLAN.md) | Active draft of a governed workspace-scoped LSP lifecycle action; later slices added approval-gated task execution and bounded process probe while state persistence and JSON-RPC remain deferred. |
| [OPENCLAW_NATIVE_ENGINEERING_LSP_SUPERVISED_LIFECYCLE_PILOT_PLAN.md](./plans/OPENCLAW_NATIVE_ENGINEERING_LSP_SUPERVISED_LIFECYCLE_PILOT_PLAN.md) | Active approval-gated LSP lifecycle task, binary gate, and supervised user-space process probe with task readback, audit events, Observer controls, and recovery recommendation while long-lived state and JSON-RPC remain deferred. |
