...
| Local validation | Workspace tests and typecheck pass; body-config, native frame/input/application lifecycle, provider flake, native inventory, event-audit integration, and the 1017-file script audit pass; 829 registry entries pass | Validated |
...
| Installed system | NixOS `26.05.4808.569d57850992`, generation `/nix/store/5zlz2s6zbcs5mhbmaskpl6njnsn7kl14-nixos-system-nixos-26.05.4808.569d57850992` | Generation 101 is active; receipt-bound semantic submit, exact local fixture isolation, and all prior physical capabilities are deployed |
| Previous generation | `/nix/store/1xh4x8ls64yzl919j2bssd9ilms98knv-nixos-system-nixos-26.05.4808.569d57850992` | Generation 100 immediately preceded the current physical switch |
| Browser network and semantic-frame correction | Browser Firefox traffic and public hostname validation use the configured loopback proxy/DoH pair; semantic type reuses the exact cached frame/inventory before a replacement capture | Physically active in `5zlz2s6z...`; Browser Runtime 31/31 and body-config checks pass, and only the exact semantic-submit fixture bypasses the private-network block |
...
| Operator credential reference | Core still receives the root-managed token through `LoadCredential`; only `edvulcan` receives read ACL on the exact token plus traverse-only ACL on its directory | Carried by `5zlz2s6z...`; every system-scope owner fixes the shared system state directory at `0750`, Core reapplies the ACL on start, the operator cannot list the directory or read the execution-grant private key, and no credential value enters environment, command line, store, or logs |
| Developer deployment | Default-off immutable helper accepts one canonical, root-owned physical `nixos-system-nixos` closure through one explicit `edvulcan` sudoers rule and rejects container markers or changes to kernel/initrd/fstab/GDM/NetworkManager/SSH paths | The fixed noninteractive helper activated `5zlz2s6z...` after every marker and protected-path check passed; current/profile match and no reboot occurred |
...
`5zlz2s6z...` contains this source fix. Level 4 now owns a compositor, headed
