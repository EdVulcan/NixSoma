...
checks. The read-only health-gate re-reads that exact staging file, reads the
current approval record, re-queries a real evaluated `/nix/store` output and
