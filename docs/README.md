...
| Browser network and semantic-frame correction | Browser Firefox traffic and public hostname validation use the configured loopback proxy/DoH pair; semantic type reuses the exact cached frame/inventory before a replacement capture | First active in `5zlz2s6z...` and carried by current `52s1asvy...`; Browser Runtime 31/31 and body-config checks pass, and only the exact semantic-submit fixture bypasses the private-network block |
