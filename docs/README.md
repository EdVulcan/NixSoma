...
| [OPENCLAW_TRUSTED_WORK_VIEW_SESSION_CONTRACT_PLAN.md](./plans/OPENCLAW_TRUSTED_WORK_VIEW_SESSION_CONTRACT_PLAN.md) | Active Level 2 bridge with user-session ownership, real NixOS Firefox, bounded visual capture, frame-grounded actions, semantic target inventory, and the next governed semantic action slice. |
