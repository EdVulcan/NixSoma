...
| Previous generation | `/nix/store/d7lxwpmj6kh4p6vgrw377lbqiq3cfrjc-nixos-system-nixos-26.05.4808.569d57850992` | Generation 99 immediately preceded the current physical switch |
| Browser network and semantic-frame correction | Browser Firefox traffic and public hostname validation use the configured loopback proxy/DoH pair; semantic type reuses the exact cached frame/inventory before a replacement capture | Physically active in `1xh4x8ls...`; browser runtime 27/27, Screen Act 8/8, Core 14/14, and body-config checks pass |
...
| Operator credential reference | Core still receives the root-managed token through `LoadCredential`; only `edvulcan` receives read ACL on the exact token plus traverse-only ACL on its directory | Deployed in `1xh4x8ls...`; every system-scope owner fixes the shared system state directory at `0750`, Core reapplies the ACL on start, the operator cannot list the directory or read the execution-grant private key, and no credential value enters environment, command line, store, or logs |
| Developer deployment | Default-off immutable helper accepts one canonical, root-owned physical `nixos-system-nixos` closure through one explicit `edvulcan` sudoers rule and rejects container markers or changes to kernel/initrd/fstab/GDM/NetworkManager/SSH paths | The fixed noninteractive helper activated `1xh4x8ls...` after every marker and protected-path check passed; current/profile match and no reboot occurred |
...
`1xh4x8ls...` contains this source fix. Level 4 now owns a compositor, headed
