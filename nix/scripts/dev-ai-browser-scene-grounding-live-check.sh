...
OBSERVER_URL="${OPENCLAW_OBSERVER_URL:-http://127.0.0.1:4173}"
...
cleanup() {
  local status="$?"
  if (( status != 0 )); then
    for diagnostic in authority-control.json activated.json single-step.json; do
...
node - "$tmp_dir" "$browser_surface_id" <<'NODE'
const fs = require("node:fs");
