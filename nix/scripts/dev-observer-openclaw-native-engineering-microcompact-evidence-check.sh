...
PROJECTION_SECRET="OBSERVER_ENGINEERING_MICROCOMPACT_PROJECTION_SECRET_DO_NOT_LEAK"
...
post_json "$CORE_URL/capabilities/invoke" '{"capabilityId":"act.openclaw.engineering_context.microcompact_projection","params":{"thresholdChars":64,"protectRecentAssistantTurns":0,"messages":[{"role":"assistant","content":[{"type":"text","text":"old"}]},{"role":"toolResult","toolName":"cc_grep","content":[{"type":"text","text":"OBSERVER_ENGINEERING_MICROCOMPACT_PROJECTION_SECRET_DO_NOT_LEAK_xxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx"}]},{"role":"assistant","content":[{"type":"text","text":"new"}]}]}}' > "$CAPABILITY_PROJECTION_FILE"

node - <<'EOF' "$HTML_FILE" "$CLIENT_FILE" "$TASK_FILE" "$STEP_FILE" "$MICROCOMPACT_FILE" "$CAPABILITY_EVIDENCE_FILE" "$CAPABILITY_PROJECTION_FILE" "$PROMPT_SECRET" "$TOOL_SECRET" "$PROJECTION_SECRET"
const fs = require("node:fs");
...
const capabilityProjection = readJson(8);
const promptSecret = process.argv[9];
const toolSecret = process.argv[10];
const projectionSecret = process.argv[11];
const rawEndpoint = JSON.stringify({ microcompact, capabilityEvidence, capabilityProjection });
...
if (
  ...
  throw new Error(`Observer common microcompact evidence capability mismatch: ${JSON.stringify(capabilityEvidence)}`);
}
if (
  ...
  throw new Error(`Observer common microcompact projection capability mismatch: ${JSON.stringify(capabilityProjection)}`);
}
if (rawEndpoint.includes("MMMMMMMMMMMMMMMMMMMM") || rawEndpoint.includes(promptSecret) || rawEndpoint.includes(toolSecret) || rawEndpoint.includes(projectionSecret)) {
