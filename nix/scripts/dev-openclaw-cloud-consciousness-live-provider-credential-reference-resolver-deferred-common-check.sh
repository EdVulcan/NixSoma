...
OBSERVER_CHECK="${PHASE34_OBSERVER_CHECK:-false}"
PORT_BASE="${PHASE34_PORT_BASE:-8780}"
...
PLAN_DOC="$REPO_ROOT/docs/plans/OPENCLAW_PHASE_34_PLAN.md"
...
export OPENCLAW_CORE_STATE_FILE="${OPENCLAW_CORE_STATE_FILE:-$REPO_ROOT/.artifacts/openclaw-core-phase-34-credential-reference-resolver-deferred-check.json}"
export OPENCLAW_SYSTEM_HEAL_STATE_FILE="${OPENCLAW_SYSTEM_HEAL_STATE_FILE:-$REPO_ROOT/.artifacts/openclaw-system-heal-phase-34-credential-reference-resolver-deferred-check.json}"
...
seed_live_provider_call_prerequisites "$CLOUD_DIR" "$PROVIDER_RESPONSE_FILE" "$RUNBOOK_FILE" "$EXECUTION_PLAN_FILE" "phase34-prereq"
cleanup() {
...
console.log(JSON.stringify({ observerOpenClawCloudConsciousnessApprovedCredentialReferenceResolverDeferred: { status: "passed", registry } }, null, 2));
EOF
  exit 0
fi
...
post_json "$CORE_URL/approvals/$approval_id/approve" '{"approvedBy":"phase-34-check","reason":"Approve deferred credential reference resolver shell."}' > "$APPROVED_FILE"
...
for (const token of [
  "openclaw-cloud-consciousness-approved-live-provider-credential-reference-resolver-deferred",
  "deferred_after_approval",
  "credential value access remains disabled",
]) {
  if (!doc.includes(token)) throw new Error(`Phase 34 plan doc missing ${token}`);
}
...
if (!step.ok || shell?.registry !== registry || shell?.implementationStatus !== "deferred_after_approval") {
  throw new Error(`Approved credential resolver task should defer after approval: ${JSON.stringify(step)}`);
}
if (!step.task?.phaseHistory?.some((entry) => entry.phase === "cloud_consciousness_live_provider_credential_reference_resolver_deferred")) {
  throw new Error(`Task should retain deferred credential resolver phase history: ${JSON.stringify(step.task?.phaseHistory)}`);
}
if (
  shell.referenceOnly !== true
  ...
  throw new Error(`Approved credential resolver task must not enable credential or live activity: ${JSON.stringify(shell)}`);
}
console.log(JSON.stringify({ openclawCloudConsciousnessApprovedCredentialReferenceResolverDeferred: { status: "passed", taskId: step.task.id, registry } }, null, 2));
EOF
