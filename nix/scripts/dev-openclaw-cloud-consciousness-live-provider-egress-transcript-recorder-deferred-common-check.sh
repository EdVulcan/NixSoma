...
OBSERVER_CHECK="${PHASE42_OBSERVER_CHECK:-false}"
PORT_BASE="${PHASE42_PORT_BASE:-8940}"
...
PLAN_DOC="$REPO_ROOT/docs/plans/OPENCLAW_PHASE_42_PLAN.md"
...
export OPENCLAW_CORE_STATE_FILE="${OPENCLAW_CORE_STATE_FILE:-$REPO_ROOT/.artifacts/openclaw-core-phase-42-egress-transcript-recorder-deferred-check.json}"
export OPENCLAW_SYSTEM_HEAL_STATE_FILE="${OPENCLAW_SYSTEM_HEAL_STATE_FILE:-$REPO_ROOT/.artifacts/openclaw-system-heal-phase-42-egress-transcript-recorder-deferred-check.json}"
...
seed_live_provider_call_prerequisites "$CLOUD_DIR" "$PROVIDER_RESPONSE_FILE" "$RUNBOOK_FILE" "$EXECUTION_PLAN_FILE" "phase42-prereq"
cleanup() {
...
console.log(JSON.stringify({ observerOpenClawCloudConsciousnessApprovedEgressTranscriptRecorderDeferred: { status: "passed", registry } }, null, 2));
EOF
  exit 0
fi
...
post_json "$CORE_URL/approvals/$approval_id/approve" '{"approvedBy":"phase-42-check","reason":"Approve deferred egress transcript recorder shell."}' > "$APPROVED_FILE"
...
for (const token of [
  "openclaw-cloud-consciousness-approved-live-provider-egress-transcript-recorder-deferred",
  "deferred_after_approval",
  "executable provider egress deferred",
]) {
  if (!doc.includes(token)) throw new Error(`Phase 42 plan doc missing ${token}`);
}
...
if (!step.ok || shell?.registry !== registry || shell?.implementationStatus !== "deferred_after_approval") {
  throw new Error(`Approved transcript recorder task should defer after approval: ${JSON.stringify(step)}`);
}
if (!step.task?.phaseHistory?.some((entry) => entry.phase === "cloud_consciousness_live_provider_egress_transcript_recorder_deferred")) {
  throw new Error(`Task should retain deferred transcript recorder phase history: ${JSON.stringify(step.task?.phaseHistory)}`);
}
if (
  ...
  throw new Error(`Approved transcript recorder task must not enable live provider activity: ${JSON.stringify(shell)}`);
}
console.log(JSON.stringify({ openclawCloudConsciousnessApprovedEgressTranscriptRecorderDeferred: { status: "passed", taskId: step.task.id, registry } }, null, 2));
EOF
