...
FIXTURE_DIR="$REPO_ROOT/.artifacts/observer-openclaw-plugin-search-web-adapter-provider-runtime-sandbox-denial-recovery-fixture"
WORKSPACE_DIR="$FIXTURE_DIR/openclaw"
PLUGIN_SDK_DIR="$WORKSPACE_DIR/packages/plugin-sdk"
SDK_SOURCE_DIR="$WORKSPACE_DIR/src/plugin-sdk"
EXTENSIONS_DIR="$WORKSPACE_DIR/extensions"
QUERY_SECRET="OBSERVER_SEARCH_WEB_SANDBOX_DENIAL_RECOVERY_QUERY_SECRET_DO_NOT_LEAK"
...
export OPENCLAW_CORE_STATE_FILE="${OPENCLAW_CORE_STATE_FILE:-$REPO_ROOT/.artifacts/openclaw-core-observer-plugin-search-web-adapter-provider-runtime-sandbox-denial-recovery-check.json}"
export OPENCLAW_EVENT_LOG_FILE="${OPENCLAW_EVENT_LOG_FILE:-$REPO_ROOT/.artifacts/observer-openclaw-plugin-search-web-adapter-provider-runtime-sandbox-denial-recovery-check-events.jsonl}"
...
{
  "name": "openclaw",
  "version": "0.0.0-observer-search-web-sandbox-denial-recovery-fixture",
  "private": true,
  "scripts": {
    "build": "echo OBSERVER_SEARCH_WEB_SANDBOX_DENIAL_RECOVERY_ROOT_SECRET_BUILD_BODY"
  }
}
JSON
cat > "$PLUGIN_SDK_DIR/package.json" <<'JSON'
{
  ...
  "version": "0.0.0-observer-search-web-sandbox-denial-recovery-sdk-fixture",
  "private": false,
  ...
  "scripts": {
    "build": "echo OBSERVER_SEARCH_WEB_SANDBOX_DENIAL_RECOVERY_SDK_SECRET_BUILD_BODY"
  }
}
JSON
cat > "$PLUGIN_SDK_DIR/src/index.ts" <<'TS'
export const OBSERVER_SEARCH_WEB_SANDBOX_DENIAL_RECOVERY_SDK_SECRET_SOURCE_CONTENT = "must-not-leak";
TS
cat > "$PLUGIN_SDK_DIR/types/index.d.ts" <<'TS'
export type ObserverSearchWebSandboxDenialRecoveryManifest = { pluginId: string };
TS
cat > "$SDK_SOURCE_DIR/core.ts" <<'TS'
export function defineObserverSearchWebSandboxDenialRecoverySourceContract() {
  return {
...
cat > "$EXTENSIONS_DIR/web/openclaw.plugin.json" <<'JSON'
{
  ...
  "providerEndpoints": [
    {
      "name": "exa",
      "hosts": ["OBSERVER_SEARCH_WEB_SANDBOX_DENIAL_RECOVERY_SECRET_ENDPOINT.example.test"]
    }
  ],
...
cat > "$EXTENSIONS_DIR/memory/openclaw.plugin.json" <<'JSON'
{
  ...
  "providers": ["lancedb"],
  "providerAuthEnvVars": {
    "lancedb": ["OBSERVER_SEARCH_WEB_SANDBOX_DENIAL_RECOVERY_SECRET_AUTH_ENV"]
  },
  "contracts": {
    "tools": ["remember"],
...
if (!blocked.ok || blocked.blocked !== true || blocked.reason !== "policy_requires_approval") {
  throw new Error(`observer provider sandbox task should block before denial: ${JSON.stringify(blocked)}`);
}
process.stdout.write(taskResponse.approval.id);
EOF
)"

post_json "$CORE_URL/approvals/$approval_id/deny" '{"deniedBy":"dev-observer-openclaw-plugin-search-web-adapter-provider-runtime-sandbox-denial-recovery-check","reason":"observer denies search/web provider sandbox fixture before recovery"}' > "$DENIED_FILE"
...
if (denied.approval?.status !== "denied" || denied.task?.status !== "failed" || denied.task?.restorable !== true) {
  throw new Error(`observer denied provider sandbox task should fail and become restorable: ${JSON.stringify(denied)}`);
}
...
if (!recovered.ok || recovered.task?.status !== "queued") {
  throw new Error(`observer recovered provider sandbox task should be queued: ${JSON.stringify(recovered)}`);
}
if (recovered.task?.type !== "openclaw_search_web_provider_runtime_sandbox") {
  throw new Error(`observer recovered task should remain provider sandbox type: ${JSON.stringify(recovered.task)}`);
}
if (recovered.task?.approval?.requestId === denied.approval?.id || recovered.task?.approval?.status !== "pending") {
  throw new Error(`observer recovered provider sandbox task should use a fresh pending approval: ${JSON.stringify(recovered.task?.approval)}`);
}
...
post_json "$CORE_URL/approvals/$recovered_approval_id/approve" '{"approvedBy":"dev-observer-openclaw-plugin-search-web-adapter-provider-runtime-sandbox-denial-recovery-check","reason":"observer approves recovered search/web provider sandbox fixture"}' > "$APPROVED_FILE"
...
if (recoveredBlocked.blocked !== true || recoveredBlocked.reason !== "policy_requires_approval") {
  throw new Error(`observer recovered provider sandbox task should block before fresh approval: ${JSON.stringify(recoveredBlocked)}`);
}
if (approved.approval?.status !== "approved" || approved.task?.policy?.decision?.decision !== "audit_only") {
  throw new Error(`observer recovered provider sandbox approval should become audited: ${JSON.stringify(approved)}`);
}
if (deferred.blocked !== true || deferred.reason !== "search_web_provider_runtime_sandbox_deferred") {
  throw new Error(`observer approved provider sandbox recovery should remain deferred: ${JSON.stringify(deferred)}`);
}
if (history.summary?.total !== 0 || history.summary?.invoked !== 0 || history.summary?.blocked !== 0) {
  throw new Error(`observer provider sandbox denial recovery must not invoke capabilities: ${JSON.stringify(history.summary)}`);
}
...
if (tasks.summary?.counts?.failed !== 1 || tasks.summary?.counts?.queued !== 1) {
  throw new Error(`observer tasks should show failed source plus deferred recovered provider sandbox task: ${JSON.stringify(tasks.summary)}`);
}
for (const secret of [
  querySecret,
  "OBSERVER_SEARCH_WEB_SANDBOX_DENIAL_RECOVERY_ROOT_SECRET_BUILD_BODY",
  "OBSERVER_SEARCH_WEB_SANDBOX_DENIAL_RECOVERY_SDK_SECRET_BUILD_BODY",
  "OBSERVER_SEARCH_WEB_SANDBOX_DENIAL_RECOVERY_SDK_SECRET_SOURCE_CONTENT",
  "OBSERVER_SEARCH_WEB_SANDBOX_DENIAL_RECOVERY_SECRET_ENDPOINT",
  "OBSERVER_SEARCH_WEB_SANDBOX_DENIAL_RECOVERY_SECRET_AUTH_ENV",
  "0.0.0-observer-search-web-sandbox-denial-recovery-fixture",
  "0.0.0-observer-search-web-sandbox-denial-recovery-sdk-fixture",
]) {
  if (raw.includes(secret)) {
    throw new Error(`Observer provider sandbox denial recovery leaked query, source, endpoint, auth env, or version detail: ${secret}`);
  }
}

console.log(JSON.stringify({
  observerOpenClawPluginSearchWebAdapterProviderRuntimeSandboxDenialRecovery: {
