...
curl --silent --fail "$CORE_URL/plugins/native-adapter/acpx-codex-bridge-live-execution-boundary-review" > "$LIVE_BOUNDARY_FILE"
curl --silent --fail "$CORE_URL/approvals?status=pending&limit=10" > "$APPROVALS_FILE"
curl --silent --fail "$CORE_URL/capabilities/invocations?limit=10" > "$HISTORY_FILE"

node - <<'EOF' "$HTML_FILE" "$CLIENT_FILE" "$FIRST_FILE" "$SECOND_FILE" "$BRIDGE_FILE" "$DRAFT_FILE" "$WRITE_PROPOSAL_FILE" "$WRITE_EXECUTION_FILE" "$PROCESS_SPAWN_PROPOSAL_FILE" "$LIVE_BOUNDARY_FILE" "$APPROVALS_FILE" "$HISTORY_FILE"
...
const liveBoundary = readJson(11);
...
for (const token of [
  ...
  "acpx-codex-bridge-live-boundary",
...
for (const token of [
  ...
  "/plugins/native-adapter/acpx-codex-bridge-live-execution-boundary-review",
...
if (
  ...
  || !liveBoundary.decision?.blockers?.includes("missing_completed_process_spawn_preflight")
  ...
  throw new Error(`Observer ACPX/Codex live execution boundary review mismatch: ${JSON.stringify(liveBoundary)}`);
}
...
console.log(JSON.stringify({
  observerOpenClawNativeAcpxCodexBridgeCompatibility: {
    ...
    liveExecutionBoundary: liveBoundary.decision.status,
