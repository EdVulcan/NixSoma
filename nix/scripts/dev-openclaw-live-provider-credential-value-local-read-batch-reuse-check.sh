...
START_PHASE="${OPENCLAW_CREDENTIAL_VALUE_LOCAL_READ_BATCH_START_PHASE:-73}"
END_PHASE="${OPENCLAW_CREDENTIAL_VALUE_LOCAL_READ_BATCH_END_PHASE:-90}"
PORT_BASE="${OPENCLAW_CREDENTIAL_VALUE_LOCAL_READ_BATCH_PORT_BASE:-28800}"
RUN_ID="${OPENCLAW_CREDENTIAL_VALUE_LOCAL_READ_BATCH_RUN_ID:-credential-value-local-read-batch-$$}"
...
export OPENCLAW_CORE_STATE_FILE="${OPENCLAW_CORE_STATE_FILE:-$REPO_ROOT/.artifacts/openclaw-core-credential-value-local-read-batch-$START_PHASE-$END_PHASE-check.json}"
export OPENCLAW_SYSTEM_HEAL_STATE_FILE="${OPENCLAW_SYSTEM_HEAL_STATE_FILE:-$REPO_ROOT/.artifacts/openclaw-system-heal-credential-value-local-read-batch-$START_PHASE-$END_PHASE-check.json}"

if (( START_PHASE < 73 || END_PHASE > 90 || START_PHASE > END_PHASE )); then
  echo "Credential-value local-read batch phase range must be within 73..90 and non-empty: $START_PHASE..$END_PHASE" >&2
  exit 1
fi
...
run_phase_common_check() {
  ...
  if [[ ! -f "$common_script" ]]; then
    echo "Missing credential-value local-read common check for phase $phase: $common_script" >&2
    exit 1
  fi
...
console.log(JSON.stringify({
  openclawCredentialValueLocalReadBatchReuse: {
    status: "passed",
