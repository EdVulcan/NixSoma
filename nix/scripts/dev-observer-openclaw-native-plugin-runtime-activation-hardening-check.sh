...
FIXTURE_DIR="$REPO_ROOT/.artifacts/observer-openclaw-native-plugin-runtime-activation-hardening-fixture"
WORKSPACE_DIR="$FIXTURE_DIR/openclaw"
PLUGIN_SDK_DIR="$WORKSPACE_DIR/packages/plugin-sdk"

export OPENCLAW_CORE_PORT="${OPENCLAW_CORE_PORT:-9330}"
export OPENCLAW_EVENT_HUB_PORT="${OPENCLAW_EVENT_HUB_PORT:-9331}"
export OPENCLAW_SESSION_MANAGER_PORT="${OPENCLAW_SESSION_MANAGER_PORT:-9332}"
export OPENCLAW_BROWSER_RUNTIME_PORT="${OPENCLAW_BROWSER_RUNTIME_PORT:-9333}"
export OPENCLAW_SCREEN_SENSE_PORT="${OPENCLAW_SCREEN_SENSE_PORT:-9334}"
export OPENCLAW_SCREEN_ACT_PORT="${OPENCLAW_SCREEN_ACT_PORT:-9335}"
export OPENCLAW_SYSTEM_SENSE_PORT="${OPENCLAW_SYSTEM_SENSE_PORT:-9336}"
export OPENCLAW_SYSTEM_HEAL_PORT="${OPENCLAW_SYSTEM_HEAL_PORT:-9337}"
export OBSERVER_UI_PORT="${OBSERVER_UI_PORT:-9400}"
export OPENCLAW_WORKSPACE_ROOTS="$WORKSPACE_DIR"
export OPENCLAW_CORE_STATE_FILE="${OPENCLAW_CORE_STATE_FILE:-$REPO_ROOT/.artifacts/openclaw-core-observer-native-plugin-runtime-activation-hardening-check.json}"
export OPENCLAW_EVENT_LOG_FILE="${OPENCLAW_EVENT_LOG_FILE:-$REPO_ROOT/.artifacts/observer-native-plugin-runtime-activation-hardening-check-events.jsonl}"
...
{
  "name": "openclaw",
  "version": "0.0.0-observer-runtime-activation-hardening-fixture",
  "private": true,
  "scripts": {
    "build": "echo OBSERVER_RUNTIME_ACTIVATION_HARDENING_ROOT_SECRET_BUILD_BODY"
  }
}
JSON
...
cat > "$PLUGIN_SDK_DIR/package.json" <<'JSON'
{
  ...
  "scripts": {
    "build": "echo OBSERVER_RUNTIME_ACTIVATION_HARDENING_SDK_SECRET_BUILD_BODY"
  }
}
JSON
cat > "$PLUGIN_SDK_DIR/src/index.ts" <<'TS'
export const OBSERVER_RUNTIME_ACTIVATION_HARDENING_SDK_SECRET_SOURCE_CONTENT = "must-not-leak";
TS
...
if (!taskResponse.ok || taskResponse.task?.type !== "native_plugin_runtime_activation") {
  throw new Error(`observer hardening should create native activation task: ${JSON.stringify(taskResponse)}`);
}
...
post_json "$CORE_URL/approvals/$approval_id/approve" '{"approvedBy":"dev-observer-openclaw-native-plugin-runtime-activation-hardening-check","reason":"observer approve native activation task"}' > "$APPROVED_FILE"
dup_approve_status="$(post_json_status "$CORE_URL/approvals/$approval_id/approve" '{"approvedBy":"dev-observer-openclaw-native-plugin-runtime-activation-hardening-check"}' "$DUP_APPROVE_FILE")"
dup_deny_status="$(post_json_status "$CORE_URL/approvals/$approval_id/deny" '{"deniedBy":"dev-observer-openclaw-native-plugin-runtime-activation-hardening-check"}' "$DUP_DENY_FILE")"
...
const blocked = JSON.parse(fs.readFileSync(process.argv[3], "utf8"));
if (!taskResponse.ok || taskResponse.task?.type !== "native_plugin_runtime_activation") {
  throw new Error(`observer recovery chain should create native activation task: ${JSON.stringify(taskResponse)}`);
}
...
post_json "$CORE_URL/approvals/$chain_approval_id/deny" '{"deniedBy":"dev-observer-openclaw-native-plugin-runtime-activation-hardening-check","reason":"observer deny native activation chain source"}' > "$DENIED_ONE_FILE"
...
post_json "$CORE_URL/approvals/$recovered_one_approval_id/deny" '{"deniedBy":"dev-observer-openclaw-native-plugin-runtime-activation-hardening-check","reason":"observer deny first recovered native activation"}' > "$DENIED_TWO_FILE"
...
if (!deniedTwo.ok || deniedTwo.task?.id !== recoveredOne.task?.id || deniedTwo.task?.status !== "failed" || deniedTwo.task?.recovery?.attempt !== 1) {
  throw new Error(`observer denying first recovered native activation should fail attempt 1: ${JSON.stringify(deniedTwo)}`);
}
...
post_json "$CORE_URL/approvals/$recovered_two_approval_id/approve" '{"approvedBy":"dev-observer-openclaw-native-plugin-runtime-activation-hardening-check","reason":"observer approve second recovered native activation"}' > "$RECOVERED_TWO_APPROVED_FILE"
...
if (deferred.blocked !== true || deferred.reason !== "native_plugin_runtime_activation_deferred") {
  throw new Error(`observer approved native activation task should remain deferred: ${JSON.stringify(deferred)}`);
}
if (recoveredTwoBlocked.blocked !== true || recoveredTwoBlocked.reason !== "policy_requires_approval") {
  throw new Error(`observer second recovered native activation should block before approval: ${JSON.stringify(recoveredTwoBlocked)}`);
}
...
if (recoveredTwoDeferred.blocked !== true || recoveredTwoDeferred.reason !== "native_plugin_runtime_activation_deferred") {
  throw new Error(`observer second recovered native activation should remain deferred: ${JSON.stringify(recoveredTwoDeferred)}`);
}
if (history.summary?.total !== 0 || history.summary?.invoked !== 0 || history.summary?.blocked !== 0) {
  throw new Error(`observer native activation hardening must not invoke capabilities: ${JSON.stringify(history.summary)}`);
}
...
if (tasks.summary?.counts?.failed !== 2 || tasks.summary?.counts?.queued !== 1) {
  throw new Error(`observer tasks should show failed chain plus deferred recovered native activation: ${JSON.stringify(tasks.summary)}`);
}
for (const secret of [
  "OBSERVER_RUNTIME_ACTIVATION_HARDENING_ROOT_SECRET_BUILD_BODY",
  "OBSERVER_RUNTIME_ACTIVATION_HARDENING_SDK_SECRET_BUILD_BODY",
  "OBSERVER_RUNTIME_ACTIVATION_HARDENING_SDK_SECRET_SOURCE_CONTENT",
  "0.0.0-observer-runtime-activation-hardening-fixture",
]) {
  if (raw.includes(secret)) {
    throw new Error(`Observer native activation hardening leaked source, script, or package detail: ${secret}`);
  }
}

console.log(JSON.stringify({
  observerOpenClawNativePluginRuntimeActivationHardening: {
