...
test("operator origin policy rejects untrusted browser origins and keeps health public", () => {
  ...
  assert.equal(auth.requestRequiresAuth(request("GET"), new URL("/operator/mission/mission-1/worklist", "http://127.0.0.1:4100")), true);
  ...
  assert.equal(auth.requestRequiresAuth(
    request("GET"),
    new URL("/proxy/session-manager/work-view/state", "http://127.0.0.1:4100"),
  ), false);
