...
import { createAiWorkspaceOcrFocusTypeCapabilityHandlers } from "./capability-runtime-ai-workspace-ocr-focus-type.mjs";
...
export function createCapabilityRuntime(deps) {
  ...
  const aiWorkspaceOcrFocusTypeHandlers = createAiWorkspaceOcrFocusTypeCapabilityHandlers({
    runtime: aiWorkspaceOcrFocusType,
  });
  ...
  const aiWorkspaceReviewedCycleHandlers = createAiWorkspaceReviewedCycleCapabilityHandlers({
    runtime: aiWorkspaceReviewedCycle,
  });
  ...
  async function dispatchCapabilityBackend(capability, request) {
    ...
    const aiWorkspaceOcrClick = await aiWorkspaceOcrClickHandlers.callBackend(
      capability,
      request,
    );
    ...
    const aiWorkspaceOcrFocusType = await aiWorkspaceOcrFocusTypeHandlers.callBackend(
      capability,
      request,
    );
    if (aiWorkspaceOcrFocusType.handled) {
      return aiWorkspaceOcrFocusType.result;
    }
    ...
    const aiWorkspaceReviewedCycle = await aiWorkspaceReviewedCycleHandlers.callBackend(
      capability,
      request,
    );
    if (aiWorkspaceReviewedCycle.handled) {
      return aiWorkspaceReviewedCycle.result;
    }
  ...
  function summariseCapabilityInvocationResult(capability, result) {
    ...
    const aiWorkspaceOcrClickSummary =
    ...
    const aiWorkspaceOcrFocusTypeSummary =
      aiWorkspaceOcrFocusTypeHandlers.summariseResult(capability, result);
    if (aiWorkspaceOcrFocusTypeSummary) {
      return aiWorkspaceOcrFocusTypeSummary;
    }
    ...
    const aiWorkspaceReviewedCycleSummary =
      aiWorkspaceReviewedCycleHandlers.summariseResult(capability, result);
    if (aiWorkspaceReviewedCycleSummary) {
      return aiWorkspaceReviewedCycleSummary;
    }
  ...
  async function invokeCapability(body = {}) {
    ...
    const aiWorkspaceOcrClickAuthorization =
      aiWorkspaceOcrClickHandlers.authorizeRequest(capability, request, body);
    if (aiWorkspaceOcrClickAuthorization.handled) {
      serverApproval = aiWorkspaceOcrClickAuthorization.authorization;
    }
    const aiWorkspaceOcrFocusTypeAuthorization =
      aiWorkspaceOcrFocusTypeHandlers.authorizeRequest(capability, request, body);
    if (aiWorkspaceOcrFocusTypeAuthorization.handled) {
      serverApproval = aiWorkspaceOcrFocusTypeAuthorization.authorization;
    }
    ...
    if (aiWorkspaceAssessmentAuthorization.handled) {
      serverApproval = aiWorkspaceAssessmentAuthorization.authorization;
    }
    ...
    const aiWorkspaceReviewedCycleAuthorization =
      aiWorkspaceReviewedCycleHandlers.authorizeRequest(capability, request, body);
    if (aiWorkspaceReviewedCycleAuthorization.handled) {
      serverApproval = aiWorkspaceReviewedCycleAuthorization.authorization;
    }
    ...
    const aiWorkspaceOcrClickValidationError =
      aiWorkspaceOcrClickHandlers.validateRequest(capability, request, body);
    if (aiWorkspaceOcrClickValidationError) {
      return {
        statusCode: 400,
        response: { ok: false, error: aiWorkspaceOcrClickValidationError },
      };
    }
    const aiWorkspaceOcrFocusTypeValidationError =
      aiWorkspaceOcrFocusTypeHandlers.validateRequest(capability, request, body);
    if (aiWorkspaceOcrFocusTypeValidationError) {
      return {
        statusCode: 400,
        response: { ok: false, error: aiWorkspaceOcrFocusTypeValidationError },
      };
    }
    ...
    if (aiWorkspaceAssessmentValidationError) {
      return {
        statusCode: 400,
        response: { ok: false, error: aiWorkspaceAssessmentValidationError },
      };
    }
    ...
    const aiWorkspaceReviewedCycleValidationError =
      aiWorkspaceReviewedCycleHandlers.validateRequest(capability, request, body);
    if (aiWorkspaceReviewedCycleValidationError) {
      return {
        statusCode: 400,
        response: { ok: false, error: aiWorkspaceReviewedCycleValidationError },
      };
    }
