...
export function createPlanBuilder(deps) {
  const { client, state, taskManager, pluginReview } = deps;
  const { fetchJson } = client;
  const { tasks, persistState, approvals, autonomyMode } = state;
  const { serialiseTask, getTaskById } = taskManager;
