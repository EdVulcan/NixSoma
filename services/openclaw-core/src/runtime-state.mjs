...
const CAPABILITY_EXECUTION_RESERVATION_TTL_MS = parseOptionalPositiveInteger(
  process.env.OPENCLAW_CAPABILITY_EXECUTION_RESERVATION_TTL_MS,
) ?? 5 * 60 * 1000;
...
const HOSTD_SOCKET_PATH = normaliseOptionalString(process.env.OPENCLAW_HOSTD_SOCKET_PATH);
...
function loadPersistentState() {
  ...
  try {
    ...
    if (Array.isArray(data?.experienceMemoryRecords)) {
      experienceMemoryRecords.clear();
      for (const record of data.experienceMemoryRecords.slice(-MAX_EXPERIENCE_MEMORY_RECORDS)) {
        if (record?.source?.taskId && record?.id && record?.schema === "openclaw.native_engineering_experience.v0") {
          experienceMemoryRecords.set(record.source.taskId, record);
        }
      }
    }
    if (Array.isArray(data?.operatorRunSessions)) {
      ...
      for (const session of data.operatorRunSessions.slice(-MAX_OPERATOR_RUN_SESSION_ENTRIES)) {
        if (typeof session?.id === "string" && session.id.trim()) {
          operatorRunSessions.set(session.id, session);
        }
      }
    }
    if (Array.isArray(data?.boundedOperatorSchedules)) {
      boundedOperatorSchedules.clear();
      for (const schedule of data.boundedOperatorSchedules.slice(-MAX_BOUNDED_OPERATOR_SCHEDULE_ENTRIES)) {
        if (typeof schedule?.id === "string" && schedule.id.trim()) {
          boundedOperatorSchedules.set(schedule.id, schedule);
        }
      }
    }
    if (Array.isArray(data?.boundedOperatorWindowLeases)) {
      boundedOperatorWindowLeases.clear();
      for (const lease of data.boundedOperatorWindowLeases.slice(-MAX_BOUNDED_OPERATOR_WINDOW_LEASE_ENTRIES)) {
        if (typeof lease?.id === "string" && lease.id.trim()) {
          boundedOperatorWindowLeases.set(lease.id, lease);
        }
      }
    }
...
function getCurrentTask() {
  ...
  return {
    tasks, approvals, runtimeState, policyAuditLog, capabilityInvocationLog, nativeEngineeringLspLifecycleRecords, nativeEngineeringPlanTodoWorkbenchRecords, acpxBridgeSessionRecords, experienceMemoryRecords, operatorRunSessions, boundedOperatorSchedules, boundedOperatorWindowLeases, fixedUnitIncidentSchedulerState, standingProviderAdvisoryState,
    ...
    MAX_NATIVE_ENGINEERING_LSP_LIFECYCLE_RECORDS, MAX_NATIVE_ENGINEERING_PLAN_TODO_WORKBENCH_RECORDS, MAX_ACPX_BRIDGE_SESSION_RECORDS, MAX_EXPERIENCE_MEMORY_RECORDS, MAX_OPERATOR_RUN_SESSION_ENTRIES, MAX_BOUNDED_OPERATOR_SCHEDULE_ENTRIES, MAX_BOUNDED_OPERATOR_WINDOW_LEASE_ENTRIES,
