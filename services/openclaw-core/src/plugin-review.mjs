...
function buildOpenClawNativePluginAdapterStatus() {
  ...
  return {
    ...
    summary: {
      ...
      canInvokeSearchWebAdapterContract: true,
    ...
    guardrails: [
      ...
      "search/web adapter contract shell is available through the dedicated route and common capability runtime without network access or runtime activation",
      ...
      "engineering microcompact projection returns a bounded transformed message copy, protects recent turns plus verification/recovery evidence, and never mutates caller input, persisted logs, task state, or provider/network boundaries",
      "engineering context packets assemble bounded task and transcript evidence with credential-like redaction, microcompaction, protected verification/recovery summaries, and summary-only audit without provider calls or network egress",
