...
export const AI_WORKSPACE_OCR_TYPE_CAPABILITY_ID =
  "act.ai.workspace.ocr_type";

function isCapability(capability) {
  return capability?.id === AI_WORKSPACE_OCR_TYPE_CAPABILITY_ID;
}

export function createAiWorkspaceOcrTypeCapabilityHandlers({ runtime } = {}) {
  function authorizeRequest(capability, request, rawBody) {
    ...
    return {
      ...
      authorization: {
        ...
        ok: approved,
        approved,
        reason: approved ? null : "ai_workspace_ocr_type_request_invalid",
        policyId: "ai-workspace-explicit-ocr-type",
        policyVersion: 0,
  ...
  function validateRequest(capability, request, rawBody) {
    ...
    if (!aiWorkspaceTaskRequestIsBounded(request, rawBody)) {
      return "AI workspace OCR type accepts only capabilityId, one taskId, and params.confirm=true.";
    }
    if (!runtime || typeof runtime.invoke !== "function") {
      return "AI workspace OCR type runtime is unavailable.";
    }
    return null;
  }
  ...
  function summariseResult(capability, result) {
    ...
    const inputEvidence = evidence.inputEvidence
      ?? result?.action?.inputEvidence
      ?? result?.decision?.inputEvidence
      ?? null;
    return {
      kind: "ai.workspace.ocr_type",
      ok: result?.ok === true,
