import { createKernelNetworkConnectReadback } from "./kernel-network-connect-readback.mjs";

const REGISTRY = "openclaw-kernel-network-connect-v0";
const TRACEPOINT = "__sys_connect";
const MAX_DURATION_MS = 5000;
const MAX_EVENTS = 4096;
const EVENT_KEYS = ["timestampNs", "pid", "uid", "comm", "family", "addressLength"];
...
function classifyProbeError(error) {
  ...
  if (code === "ETIMEDOUT" || error?.signal === "SIGTERM" || message.includes("timed out")) {
    return {
      code: "timeout",
      message: "Kernel network-connect probe exceeded its bounded capture window.",
    };
  }
  return {
    code: "unavailable",
    message: "Kernel network-connect probe could not be executed.",
  };
}
...
function parseEvents(stdout, maxEvents) {
  ...
  return lines.map((line) => {
    ...
    if (!Number.isInteger(value.family) || value.family < 0 || value.family > 65535) {
      throw invalidOutput("probe output address family was invalid");
    }
    if (!Number.isInteger(value.addressLength) || value.addressLength < 0 || value.addressLength > 65535) {
      throw invalidOutput("probe output address length was invalid");
    }
    return {
      ...
      comm: value.comm,
      family: value.family,
      addressLength: value.addressLength,
    };
  });
}

export function createKernelNetworkConnectCapture({
  enabled = false,
  probeCommand = "",
  durationMs = 1000,
  maxEvents = 128,
  ...
  async function captureNow() {
    ...
    if (typeof probeCommand !== "string" || probeCommand.trim() === "" || typeof runProbe !== "function") {
      return buildReadModel({
        ...
        error: {
          ...
          message: "Kernel network-connect probe is not configured.",
        },
      });
    }
    ...
    try {
      ...
      const classified = error?.code === "invalid_output"
        ? { code: "invalid_output", message: "Kernel network-connect probe returned invalid event data." }
  ...
  async function capture() {
    if (activeCapture) {
      return buildReadModel({
        ...
        error: {
          code: "busy",
          message: "Kernel network-connect probe capture is already in progress.",
        },
      });
    }
...
export const KERNEL_NETWORK_CONNECT_REGISTRY = REGISTRY;
