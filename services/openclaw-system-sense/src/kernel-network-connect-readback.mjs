const REGISTRY = "openclaw-kernel-network-connect-readback-v0";
const CONTINUITY_REGISTRY = "openclaw-kernel-network-connect-continuity-v0";
const MAX_COMMAND_SUMMARY_ENTRIES = 16;
const MAX_FAMILY_SUMMARY_ENTRIES = 16;
const MAX_NEW_COMM_NAMES = 16;
const MAX_TRACKED_COMM_NAMES = 64;

export function buildKernelNetworkConnectReadback({
  events = [],
  ...
  for (const event of events) {
    ...
    if (Number.isInteger(event.family)) {
  ...
  const sortedFamilyCounts = [...familyCounts.entries()]
    .sort(([leftFamily, leftCount], [rightFamily, rightCount]) => (
      rightCount - leftCount || leftFamily - rightFamily
    ));
  ...
  return {
    ...
    uniqueFamilyCount: familyCounts.size,
    ...
    familyCounts: sortedFamilyCounts
      .slice(0, MAX_FAMILY_SUMMARY_ENTRIES)
      .map(([family, count]) => ({ family, count })),
    familyCountsTruncated: sortedFamilyCounts.length > MAX_FAMILY_SUMMARY_ENTRIES,
...
export function createKernelNetworkConnectReadback({
  ...
  return ({
    ...
    const currentActivity = events.length > 0 ? "connect_attempts_observed" : "quiet";
...
export const KERNEL_NETWORK_CONNECT_READBACK_REGISTRY = REGISTRY;
export const KERNEL_NETWORK_CONNECT_CONTINUITY_REGISTRY = CONTINUITY_REGISTRY;
